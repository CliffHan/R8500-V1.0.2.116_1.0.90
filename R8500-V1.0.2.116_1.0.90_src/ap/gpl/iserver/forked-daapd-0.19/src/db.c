...
#undef Q_TMPL
	...
	/*Foxconn added start by Kathy @ check guid exists from backup songs.db files */
int found_guid(char *guid_value)
{
    ...
    sprintf(dev_guid,"%s",guid_value);
    for(db_idx=0;db_idx<32;db_idx++)
    {
        sprintf(filePath,"/tmp/backup_dbfile%d",db_idx);
        if(access(filePath,F_OK)==0)
        {
            sprintf(buffer,"rm -rf /tmp/backup_gui_info");
            system(buffer);
            sprintf(buffer,"cat %s | grep %s > /tmp/backup_gui_info",filePath, dev_guid);
            system(buffer);
            sprintf(buffer,"rm -rf /tmp/backup_dbfile%d",db_idx);
            system(buffer);

            fp=fopen("/tmp/backup_gui_info","r");
            ...
            if(fp!=NULL)
            {
                while(!feof(fp))
                {
                    ...
                    if(strstr(buffer, dev_guid)!=NULL)
                    {
                        resp=1;
                        break;
                    }
...
/*Foxconn added end by Kathy @ check guid exists from backup songs.db files */    
int
  ...
  if (ret != SQLITE_ROW)
    {
        if (ret == SQLITE_DONE)
        {
            if(found_guid(pi->guid)) /*Foxconn added by Kathy @ check guid exists from backup songs.db files */
            {
                sqlite3_finalize(stmt);
                sqlite3_free(query);    
                return 0;
...
DPRINTF(E_FATAL, L_DB, "db_path=%s\n",db_path);
  ...
  if (ret < 0)
  {
      DPRINTF(E_FATAL, L_DB, "thread create failed\n");
    return ret;
  }
  ...
  db_perthread_deinit();
      DPRINTF(E_FATAL, L_DB, "Database ok\n");
