...
//#define	ET_MSG(args) printk args
...
/*foxconn Han edited start 10/26/2016, from CSP#1063023 for frame error between internal/external switch*/
/* Internal GPHY MII registers */
#define REG_GPHY_MII_CTL	0x0
#define REG_GPHY_MII_EXT_CTL	0x20	/* PHY Extended Control register */
#define REG_GPHY_MII_EXT_STS	0x22	/* PHY Extended Status register */
...
/*foxconn Han edited end 10/26/2016, from CSP#1063023 for frame error between internal/external switch*/
...
/*Foxconn modified,edward,zhang,2013/07/03*/
#ifndef VLAN_SUPPORT
...
/*foxconn Han edited, 05/11/2015 for 2nd switch*/
#ifdef CONFIG_2ND_SWITCH
static robo_info_t *erobo_ptr = NULL;    
...
#ifdef ETEROBO
#define NS_CHIPCB_MII		(0x18003000)
#define SET_ROBO_CCB_MIIREGS(erobo)	((erobo)->miiregs = \
	(ccb_miiregs_t *)REG_MAP(NS_CHIPCB_MII, SI_CORE_SIZE))
#define SET_ROBO_CCB_MIIOPS(robo)	((robo)->ops = &ccb_mdcmdio)

/* MII mgt register */
#define CCB_MIIMGT_EXT		(1 << 9)	/* Set MII interface to external pins */
#define CCB_MIIMGT_BSY		(1 << 8)	/* Busy */
#define CCB_MIIMGT_PRE		(1 << 7)	/* Preamble */
#define CCB_MIIMGT_MDCDIV	(0x1a)		/* 2.5M clk */

/* MII cmd register */
#define CCB_MIICMD_SB		(1 << 30)	/* Start bit */
#define CCB_MIICMD_WR		(1 << 28)	/* OP: write */
#define CCB_MIICMD_RD		(2 << 28)	/* OP: read */
#define CCB_MIICMD_PA_MSK	(0x1f)		/* PHY address mask */
#define CCB_MIICMD_PA(pa)	(((pa) & CCB_MIICMD_PA_MSK) << 23)	/* PHY address */
#define CCB_MIICMD_REG_MSK	(0x1f)		/* PHY register mask */
#define CCB_MIICMD_REG(reg)	(((reg) & CCB_MIICMD_REG_MSK) << 18)	/* PHY register */
#define CCB_MIICMD_TA		(2 << 16)	/* Turn arround bit */
#define CCB_MIICMD_DATA_MSK	(0xffff)	/* Data mask */
...
typedef struct erobo_info {
	robo_info_t robo;
	ccb_miiregs_t *miiregs;
} erobo_info_t;

/* MII interface functions */
static dev_ops_t ccb_mdcmdio = {
	NULL,
	NULL,
	mii_wreg,
	mii_rreg,
	"chipcommonb MII (MDC/MDIO)"
};

uint16
ccb_miird(void *h, uint phyaddr, uint reg)
{
	erobo_info_t *erobo;
	void *osh;
	uint32 val32;
	...
	erobo = (erobo_info_t *)h;
	osh = si_osh(erobo->robo.sih);

	/* Set CCB MII to external pins and configure MDC */
	val32 = (CCB_MIIMGT_EXT |
		CCB_MIIMGT_PRE |
		CCB_MIIMGT_MDCDIV);
	W_REG(osh, &erobo->miiregs->mgt, val32);

	SPINWAIT(((val32 = R_REG(osh, &erobo->miiregs->mgt)) & CCB_MIIMGT_BSY), 10000);
	if (val32 & CCB_MIIMGT_BSY) {
		ET_ERROR(("ccb_miird: busy 1\n"));
	}

	/* Read PHY register */
	val32 = (CCB_MIICMD_SB |
		CCB_MIICMD_RD |
		CCB_MIICMD_PA(phyaddr) |
		CCB_MIICMD_REG(reg) |
		CCB_MIICMD_TA);
	ET_MSG(("ccb_miird: write cmd 0x%x\n", val32));
	W_REG(osh, &erobo->miiregs->cmd, val32);

	SPINWAIT(((val32 = R_REG(osh, &erobo->miiregs->mgt)) & CCB_MIIMGT_BSY), 10000);
	if (val32 & CCB_MIIMGT_BSY) {
		ET_ERROR(("ccb_miird: busy 2\n"));
	}
	val32 = R_REG(osh, &erobo->miiregs->cmd);

	ET_MSG(("ccb_miird: read cmd 0x%x\n", val32));
	return (uint16)(val32 & CCB_MIICMD_DATA_MSK);
}

void
ccb_miiwr(void *h, int phyaddr, int reg, uint16 val)
{
	erobo_info_t *erobo;
	void *osh;
	uint32 val32;
	...
	val32 = (CCB_MIICMD_SB |
		...
		CCB_MIICMD_TA |
		val);
	ET_MSG(("ccb_miiwr: write cmd 0x%x\n", val32));
	W_REG(osh, &erobo->miiregs->cmd, val32);

	SPINWAIT(((val32 = R_REG(osh, &erobo->miiregs->mgt)) & CCB_MIIMGT_BSY), 10000);
	if (val32 & CCB_MIIMGT_BSY) {
		ET_ERROR(("ccb_miiwr: busy 2\n"));
	}
}

robo_info_t *
bcm_erobo_attach(si_t *sih, char *vars)
{
	erobo_info_t *erobo;
	robo_info_t *robo;
	uint32 dmu_base;
	...
	if (!sih || !BCM4707_CHIP(sih->chip)) {
		ET_ERROR(("bcm_erobo_attach: chip not matched\n"));
		return NULL;
	}

	/* Configure GPIO PIN MUX to enable CCB MII function */
	...
	if (!(erobo = MALLOC(si_osh(sih), sizeof(erobo_info_t)))) {
		ET_ERROR(("bcm_erobo_attach: out of memory, malloced %d bytes\n",
		          MALLOCED(si_osh(sih))));
		return NULL;
	}
	bzero(erobo, sizeof(erobo_info_t));

	robo = (robo_info_t *)erobo;
	robo->h = erobo;
	robo->sih = sih;
	robo->vars = vars;
	robo->miird = ccb_miird;
	robo->miiwr = ccb_miiwr;
	robo->page = -1;

	SET_ROBO_CCB_MIIREGS(erobo);
	SET_ROBO_CCB_MIIOPS(robo);
	...
	printf("bcm_erobo_attach devid = 0x%x\n", robo->devid);

	return (robo_info_t *)erobo;
}

void
bcm_erobo_detach(robo_info_t *robo)
{
	erobo_info_t *erobo;
	...
	if (BCM4707_CHIP(CHIPID(robo->sih->chip)) && !ROBO_IS_BCM5301X(robo->devid)) {
		erobo = (erobo_info_t *)robo;
		REG_UNMAP(erobo->miiregs);
		MFREE(si_osh(robo->sih), robo, sizeof(erobo_info_t));
	}
...
/*foxconn Han edited start 10/26/2016, from CSP#1063023 for frame error between internal/external switch*/
/* need to apply the internal GPHY setting everytime after phy reset */
void
bcm_robo_gphy_config(robo_info_t *robo, int port)
{
	...
	printf("%s: gphy config done for port %d\n", __FUNCTION__, port);
}
...
robo_bhdr_register(robo_info_t *robo, bhdr_ports_t bhdr_port, uint8 registrant)
{
	...
	if (!robo || registrant == 0)
		return -1;

	if (!BCM4707_CHIP(CHIPID(robo->sih->chip)))
		return -1;

	if (bhdr_port >= MAX_BHDR_PORTS)
		return -1;
...
bcm_robo_config_vlan(robo_info_t *robo, uint8 *mac_addr)
{
	...
	if (BCM4707_CHIP(CHIPID(robo->sih->chip)) && !ROBO_IS_BCM5301X(robo->devid))
		is_erobo = TRUE;
	ET_MSG(("bcm_robo_config_vlan: is_erobo %d\n", is_erobo));
#endif /* ETEROBO */
...
#ifdef ETEROBO
		if (is_erobo)
			sprintf(vlanports, "evlan%dports", vid);
		else
#endif /* ETEROBO */
		/* get vlan member ports from nvram */
		sprintf(vlanports, "vlan%dports", vid);
...
vlan_setup:
        ...
        if(val32)
            printk(KERN_EMERG"%s %d is_erobo=%d vid=%d untag|member=0x%X ports=%s<\n",__func__,__LINE__,is_erobo,vid,val32,ports);
...
/*foxconn Han edited start, 05/26/2015 from BRCM Cathy dump register status*/
#ifdef ETEROBO
void
_robo_dump_vlan_table(robo_info_t *robo)
{
	uint8 val8;
	uint16 val16;
	uint32 val32;
	...
	for (i = 0; i <= VLAN_MAXVID; i++) {
		val16 = i;  /* vlan id */
		robo->ops->write_reg(robo, PAGE_VTBL, REG_VTBL_INDX_5395, &val16,
                       sizeof(val16));

		val8 = ((1 << 7) |  /* start command */
           1);  /* read */
		robo->ops->write_reg(robo, PAGE_VTBL, REG_VTBL_ACCESS_5395, &val8,
                       sizeof(val8));

		robo->ops->read_reg(robo, PAGE_VTBL, REG_VTBL_ENTRY_5395, &val32,
                      sizeof(val32));
        if(val32)
    		printk("VLAN %d untag bits: 0x%02x member bits: 0x%02x\n",
              i, (val32 & 0x3fe00) >> 9, (val32 & 0x1ff));
	}
}

void robo_dump_vlan_status(void)
{
	robo_info_t *robo = robo_ptr;
	
	printk("\nInternal switch:\n");
	if(robo)
		_robo_dump_vlan_table(robo);
	
	#ifdef CONFIG_2ND_SWITCH
    robo_info_t *erobo = erobo_ptr;
	printk("\nExternal switch:\n");
	if(erobo)
		_robo_dump_vlan_table(erobo);
	#endif /*CONFIG_2ND_SWITCH*/

}
#endif /*ETEROBO*/
/*foxconn Han edited end, 05/26/2015 from BRCM Cathy dump register status*/
...
int robo_read_link_status(int lan_wan, int *link, int *speed, int *duplex)
{
    ...
    if(erobo)
    {
	    ...
	    erobo->ops->read_reg(erobo, PAGE_STATUS, REG_LINK_SUM, &ereg_link, sizeof(ereg_link));

    	/* Read port speed summary register */
    	erobo->ops->read_reg(erobo, PAGE_STATUS, REG_SPEED_SUM, &ereg_speed, sizeof(ereg_speed));
	
    	/* Read duplex status summary register */
	    erobo->ops->read_reg(erobo, PAGE_STATUS, REG_DUPLEX_SUM, &ereg_duplex, sizeof(ereg_duplex));

        //printk(KERN_EMERG"%s erobo 0x%X 0x%X 0x%X\n",__func__,ereg_link,ereg_speed,ereg_duplex);
    }
    #endif /*CONFIG_2ND_SWITCH*/
    ...
    //printk(KERN_EMERG"%s robo 0x%X 0x%X 0x%X\n",__func__,reg_link,reg_speed,reg_duplex);

    if (lan_wan == 0) /*lan status*/
    {
        *speed = 0;

        /*foxconn Han edited start, 05/12/2015*/
        #ifdef CONFIG_2ND_SWITCH
        if(ext)
        {
            *link = (reg_link & ROBO_LAN_PORTMAP_FOR_EXT);
            //printk(KERN_EMERG"%s 0x%X => ",__func__,*link);
            *link |= (ereg_link & EXT_ROBO_LAN_PORTMAP)<<EXT_PORT_SHIFT;
            //printk(KERN_EMERG" | 0x%X = 0x%X\n",((ereg_link & EXT_ROBO_LAN_PORTMAP)<<EXT_PORT_SHIFT),*link);

            /*mask the reg_link prevent misuse*/
            reg_link = (reg_link & ROBO_LAN_PORTMAP_FOR_EXT);
            ereg_link = (ereg_link & EXT_ROBO_LAN_PORTMAP);
        }
        else
        #endif /*CONFIG_2ND_SWITCH*/
        /*foxconn Han edited end, 05/12/2015*/
        *link = (reg_link & ROBO_LAN_PORTMAP);
        ...
        for (i = ROBO_LAN_PORT_IDX_START; i <= ROBO_LAN_PORT_IDX_END; i++) {
            ...
            #else
            /* BCM53115S Giga phy support*/
            port_speed = (reg_speed >> (2*i)) & 0x03;
            #endif
            /* Foxconn add end by aspen Bai, 10/09/2008 */
            if (port_link && (port_speed > *speed))
            {
                //printk(KERN_EMERG"%s %d %d 0x%X 0x%X 0x%X\n",__func__,__LINE__,i,port_link,port_speed, *speed);
                *speed = port_speed;
            }
        }

        //printk(KERN_EMERG"%s %d link=0x%X speed=0x%X\n",__func__,__LINE__,*link,*speed);
        /*foxconn Han edited start, 05/12/2015*/
        #ifdef CONFIG_2ND_SWITCH
        if(ext)
        {
            for (i = ROBO_LAN_PORT_IDX_START; i <= ROBO_LAN_PORT_IDX_END; i++) {
                port_link  = (ereg_link >> i) & 0x01;
                /* Foxconn add start by aspen Bai, 10/09/2008 */
                #if (defined BCM4716) || (defined BCM5356) || (defined BCM5325E)
                /* BCM5325E & BCM5325F */
                port_speed = (ereg_speed >> i) & 0x01;
                #else
                /* BCM53115S Giga phy support*/
                port_speed = (ereg_speed >> (2*i)) & 0x03;
                #endif
                /* Foxconn add end by aspen Bai, 10/09/2008 */
                if (port_link && port_speed > *speed)
                {
        ...
        /* foxconn Han noted, 05/12/2015 
         * we don't use duplex now, so didn't update duplex from 2nd switch*/
