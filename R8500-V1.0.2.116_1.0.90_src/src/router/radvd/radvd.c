/*
 *   $Id: radvd.c 241182 2011-02-17 21:50:03Z $
 *
 *   Authors:
...
main(int argc, char *argv[])
{
	...
	char *username = NULL;
	char *chrootdir = NULL;
	int boolDaemon= 0; //Added to force daemon with debug turned on.
#ifdef HAVE_GETOPT_LONG
	int opt_idx;
#endif
	...
	/* parse args */
#ifdef HAVE_GETOPT_LONG
	while ((c = getopt_long(argc, argv, "d:C:l:m:p:t:u:vhD", prog_opt, &opt_idx)) > 0) //Added D option.
#else
	while ((c = getopt(argc, argv, "d:C:l:m:p:t:u:vhD")) > 0) //Added D option.
#endif
	{
		switch (c) {
		case 'C':
		...
		case 'd':
			...
			break;
		case 'D': //Added to daemonize regardless of debug level.
			boolDaemon= 1;
			break;
		case 'f':
	...
	if (get_debuglevel() == 0 || boolDaemon) {
